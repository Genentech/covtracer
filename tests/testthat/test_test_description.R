test_that("test_description can build a test descriptions a call with a srcref", { # nolint
  # parsing from an expression with src file
  parse_content <- parse(
    file = file.path(test_path(), "test_test_description.R"), keep.source = TRUE
  )
  ...
  # full expression
  expect_match(
    test_description(parse_content), "test_description can build"
  )
  # a single srcref
  expect_match(
    test_description(x_srcref), "\\[test_test_description.R#L\\d+\\]"
  )
  # a single srcref
  expect_match(
    test_description(x_srcref), "test_that\\(\"test_description can build"
  )

  # parsing from an expression without src file
  parse_content <- parse(
    file = file.path(test_path(), "test_test_description.R"),
    keep.source = FALSE
  )
  expect_equal(getSrcref(parse_content), NULL) # content without srcref
  # does not start with file name
  expect_match(test_description(parse_content), "^[^[]")
  expect_match(test_description(parse_content), "test_description can build")
})

# leave `testthat::` - used for this test
testthat::test_that("test_description can build a test description from testthat::test_that call", { # nolint
  # parsing from an expression with src file
  parse_content <- parse(
    file = file.path(test_path(), "test_test_description.R"),
    keep.source = TRUE
...
test_that("test_description can extract test descriptions from a call stack containing a testthat call", { # nolint
...
test_that("test_description handles errors while evaluating non-atomic test descriptions", { # nolint
  expr <- list(as.call(quote(test_that(1L + "test", { }))))
  expect_equal(as.character(test_description(expr)), "1L + \"test\"")
  expect_equal(attr(test_description(expr), "type"), "testthat")
})

test_that("test_description can extract test descriptions from a non-testthat call", { # nolint
...
test_that("test_description safely handles non-standard input by returning NA", { # nolint
  expr <- NULL
  expect_equal(test_description(expr), NA_character_)
  expect_null(attr(test_description(expr), "type"))
})

test_that("test_description can extract testthat describe-it style test descriptions", { # nolint
  n_calls <- length(sys.calls())
  log_call_stack <-
    function() utils::head(utils::tail(sys.calls(), -n_calls), -1L)
  ...
  expect_equal(
    as.character(desc),
    "abc: does the thing: ... and this behavior: does another thing"
  )
  expect_equal(attr(desc, "type"), "testthat")
})
...
test_that("test descriptions can be extracted from implicit generic calls", {
  ...
  expect_equal(
    as.character(s4generic_test_desc), "S4 Generic Call: show(<myS4Example>)"
  )
})
