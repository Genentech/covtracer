...
#' \item{file}{A \code{character} filename of the Rd file in the
#'  "man" directory}
...
Rd_df <- function(x) {
  ...
  # avoid OS-specific file sorting
  rd_names <- sort(names(db))
  db <- db[rd_names]
  ...
  # avoid OS-specific file sorting
  aliases <- aliases[rd_names]
  doctype <- doctype[rd_names]
