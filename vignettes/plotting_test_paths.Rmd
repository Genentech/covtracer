...
warning(paste0(collapse = "\n", capture.output(test_names)))
warning(paste0(collapse = "\n", capture.output(obj_names)))
...
test_vertices <- data.frame(
  name = test_names,
  color = rep_len("cornflowerblue", n_tests),
  label = sprintf("Test #%d", seq_along(test_names)),
  test_id = seq_along(test_names),
  is_test = rep_len(TRUE, n_tests),
  is_exported = rep_len(NA, n_tests)
)

obj_vertices <- data.frame(
  name = obj_names,
  color = rep_len("darkgoldenrod", n_objs),
  label = rep_len(obj_names, n_objs),
  test_id = rep_len(NA, n_objs),
  is_test = rep_len(FALSE, n_objs),
